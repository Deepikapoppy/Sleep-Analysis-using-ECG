"""
=============================================================================
phase1_preprocessing_testing/p1_plots.py
All Phase-1 visualisation functions — IDENTICAL logic to the training
pipeline (fully signal-agnostic, no dataset-specific branching in here at
all). Only the internal import was switched to package-relative.
...
from .p1_dwt_denoise import _dwt_subbands
...
def plot_spectral_comparison(raw_ecg, clean_ecg, fs, config, logger,
    ...
    max_plot_seconds = config.get("spectral_plot_max_sec", 120)
    max_samples      = int(fs * max_plot_seconds)
    if len(raw_ecg) > max_samples:
        start = max(0, (len(raw_ecg) - max_samples) // 2)
        raw_ecg   = raw_ecg[start:start + max_samples]
        clean_ecg = clean_ecg[start:start + max_samples]
        logger.info(
            f"  PSD plot uses {len(raw_ecg)} samples "
            f"({max_plot_seconds}s) from the middle of the recording"
        )
