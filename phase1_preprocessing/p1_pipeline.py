"""
=============================================================================
phase1_preprocessing/p1_pipeline.py
Orchestrates all Phase-1 steps for one record.
Marks phase1_done in SQLite on success / failure.

Full pipeline
─────────────
Step 1  p1_load_ecg             → load raw ECG
Step 2  p1_downsample           → resample to 125 Hz
...
from phase1_preprocessing.p1_logging            import setup_logger, log_phase_header, log_step
from phase1_preprocessing.p1_load_ecg           import load_ecg, extract_recording_start_time
from phase1_preprocessing.p1_downsample         import downsample
from phase1_preprocessing.p1_dwt_denoise        import dwt_filter
from phase1_preprocessing.p1_polarity_correction import detect_and_apply_global_polarity
from phase1_preprocessing.p1_zscore_normalisation import normalize
from phase1_preprocessing.p1_segmentation       import segment_epochs
from phase1_preprocessing.p1_sqi                import compute_sqi_all_epochs, detect_bad_epochs
from phase1_preprocessing.p1_plots              import (
...
from phase1_preprocessing.p1_save               import save_preprocessed
...
def run_phase1_record(config: dict,
    ...
    """
    Run full Phase-1 pipeline for config['record_name'].
    ...
    try:
        ...
        stages = [
            ("1. Raw (downsampled to 125 Hz)",                 ecg_ds),
...
if __name__ == "__main__":
    ...
    logger  = setup_logger("phase1", log_dir="logs", log_file="phase1.log")
    records = get_all_records(CONFIG["dataset"])   # reads from SQLite
    ...
    for row in records:
        ...
        run_phase1_record(CONFIG, logger)
