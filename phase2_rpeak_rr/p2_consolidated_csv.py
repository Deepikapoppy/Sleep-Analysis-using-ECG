"""
=============================================================================
phase2_rpeak_rr/p2_consolidated_csv.py
Build a consolidated CSV across ALL records.

Run this AFTER p2_pipeline.py has completed for all records.

Output files
────────────
results/consolidated/
    phase2_consolidated_all_records.csv   ← one row per record
...
def build_consolidated_csv(config: dict,
    ...
    logger      : logger
    output_root : root of results/ folder
    ...
    records = get_all_records(dataset)

    if not records:
        logger.warning("No records found in SQLite — check Phase 0 ran correctly.")
        return pd.DataFrame()

    logger.info(f"Building consolidated CSV for {len(records)} {dataset} records...")

    rows = []
    missing = []
    for rec in records:
        rec_name  = rec["record_name"]
...
if __name__ == "__main__":
    ...
    from phase2_rpeak_rr.p2_logging           import setup_logger

    logger = setup_logger("consolidated", log_dir="logs",
                          log_file="phase2_consolidated.log")
    df = build_consolidated_csv(CONFIG, logger,
                                 output_root=CONFIG.get("results_dir", "results"))
